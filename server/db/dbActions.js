...
savePaymentToken: async (shopId, payToken) => {
    try {
        ...
        // Restore all non-owner shop users back to active state upon successful payment update
        await db.query(
...
processAccountPayment: async (shopId, calculatedFee = 1200.00) => {
    try {
        ...
        // Restore all non-owner members of the shop back to active status
        await db.query(
            `UPDATE users SET status = 'active' WHERE sid = ? AND usertype != 'owner'`,
            [shopId]
        );
...
applyDailyLiabilitiesAndCheckLocks: async () => {
    try {
        // 1. Lock the owners who exceeded the grace period
        ...
        // 2. Set status to 'hold' for all non-owner users belonging to locked shops
        const holdOtherUsersQuery = `
        UPDATE users u
        JOIN users o ON u.sid = o.sid AND o.usertype = 'owner'
        SET u.status = 'hold'
        WHERE u.usertype != 'owner' AND o.status = 'locked'
        `;
...
handleFailedAutopay: async (shopId, fee) => {
    try {
        ...
        // If the shop became locked due to the failure, put all non-owner users on hold
        await db.query(`
        UPDATE users 
        SET status = 'hold' 
        WHERE sid = ? AND usertype != 'owner' 
...
saveSubscriptionPayment: async (shopId, amount, paymentOrderId) => {
    try {
        ...
        // Restore all non-owner users belonging to this shop back to 'active' status upon full subscription payment
        await db.query(
            `UPDATE users SET status = 'active' WHERE sid = ? AND usertype != 'owner'`,
            [shopId]
        );
